...
```
~/.claude/skills/slack/config.env
```
...
```bash
source ~/.claude/skills/slack/config.env
```

URL이 비어있거나 파일이 없으면:
- "Webhook URL이 설정되지 않았습니다. `~/.claude/skills/slack/config.env`에 SLACK_WEBHOOK_URL을 설정해주세요." 출력 후 중단
